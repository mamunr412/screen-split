...
const LeftRightLayout = styled.div.attrs(({ leftWidth, rightWidth }: any) => ({
  style: {
    gridTemplateColumns: `${leftWidth} auto ${rightWidth}`,
  },
}))`
  ...
  grid-template-rows: 1fr;
`;
const TopBottomLayout = styled.div.attrs(({ topHeight,bottomHeight }:any) => ({
  style: {
    gridTemplateColumns: `${topHeight} auto ${bottomHeight}`,
  }
}))`
  ${fullDivCss}
  display: grid;
