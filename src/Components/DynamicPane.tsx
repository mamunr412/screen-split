import {
  ...
  Split,
  RenderSplitterProps,
} from "@geoffcox/react-splitter";
import styled, { css } from "styled-components";
import { v4 as uuidv4 } from "uuid";
import { useRecoilCallback, useRecoilState, useRecoilValue } from "recoil";
import { createSplitOptions, splitStateFamily } from "../Types/appModel";
import { SplitNode } from "../Types/types";
import {
...
export const DynamicPane = (props: Props) => {
  ...
  const onRemoveChildPane = useRecoilCallback(
    ({ snapshot, set, reset }) =>
      async (childId: string) => {
        ...
        const remainingId =
          node?.primaryId === childId
            ? node.secondaryId
            : node?.secondaryId === childId
            ? node.primaryId
            : undefined;
        ...
        const remainingNode = await snapshot.getPromise(
          splitStateFamily(remainingId)
        );
    ...
    return (
      <ActionsArea>
        <ActionButtons>
          ...
          {onToggleParentSplit && (
            <ActionButton
              onClick={() => onToggleParentSplit?.()}
              title="Toggle Parent Split"
            >
              /
            </ActionButton>
          )}
      ...
      options && (
        ...
        >
          {primaryId ? (
            ...
            <DynamicPane
              id={primaryId}
              onRemove={onRemoveChildPane}
              onToggleParentSplit={onToggleSplit}
            />
          ) : (
            // </div>
            <div>ERROR</div>
  ...
  const renderLayout = () => {
    return options
      ? options.horizontal
        ? renderTopBottomSplit()
        : renderLeftRightSplit()
      : renderActions();
  };
